...
// 95 ASCII characters with 5 bytes each
const uint8_t fonts[95 * 5] = {
...
void gfx_draw_char(uint16_t x, uint16_t y, const char c, uint16_t colour, uint16_t bg_colour) {
    // convert char into idx in the fonts array
    uint8_t idx = (c-32)*5;

    // loop through each pixel using fonts array
    for (uint8_t i = 0; i < FONT_WIDTH; i++) {
        for (uint8_t j = 0; j < FONT_HEIGHT; j++) {
            if ((fonts[idx + i] >> j) & 1) {
                lcd_draw_pixel(x+i, y+j, colour);
            } else {
                lcd_draw_pixel(x+i, y+j, bg_colour);
            }
        }
    }
}
...
void gfx_draw_string(uint16_t x, uint16_t y, const char * s, uint16_t colour, uint16_t bg_colour) {
    uint8_t i = 0;
    while (s[i] != '\0') {
        gfx_draw_char(x, y, s[i], colour, bg_colour);
        x += FONT_WIDTH;
        lcd_fill_rect(x, y, FONT_SPACING, FONT_HEIGHT, bg_colour);
        x += FONT_SPACING;
        i++;
    }
}
...
void gfx_draw_sprite(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint8_t * sprite, uint16_t colour, uint16_t bg_colour) {
    // loop through each pixel using sprite array
